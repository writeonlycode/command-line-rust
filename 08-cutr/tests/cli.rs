...
fn dies_bad_digit_field() -> Result<()> {
    ...
    dies(&[CSV, "-f", &bad], r#"invalid digit found in string"#)
}
...
fn dies_bad_digit_bytes() -> Result<()> {
    let bad = random_string();
    dies(&[CSV, "-b", &bad], r#"invalid digit found in string"#)
}
...
fn dies_bad_digit_chars() -> Result<()> {
    let bad = random_string();
    dies(&[CSV, "-c", &bad], r#"invalid digit found in string"#)
}
...
fn dies_empty_delimiter() -> Result<()> {
    dies(
        &[CSV, "-f", "1", "-d", ""],
        r#"cannot parse char from empty string"#,
    )
}
...
fn dies_bad_delimiter() -> Result<()> {
    dies(
        &[CSV, "-f", "1", "-d", ",,"],
        r#"too many characters in string"#,
    )
}
